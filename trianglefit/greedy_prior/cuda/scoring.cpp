...
std::vector<torch::Tensor> score_triangles(
  ...
  TORCH_CHECK(attention.is_cuda(), "attention must be a CUDA tensor");
  ...
  TORCH_CHECK(attention.scalar_type() == torch::kFloat32, "attention must be float32");
  ...
  TORCH_CHECK(attention.is_contiguous(), "attention must be contiguous");
  ...
  TORCH_CHECK(attention.numel() == 0 || (attention.dim() == 2 && attention.size(0) == target.size(1) && attention.size(1) == target.size(2)), "attention must be empty or have shape [H, W]");
  ...
  return score_triangles_cuda(target, current, attention, centers, half_base, height, theta, current_sse);
}
